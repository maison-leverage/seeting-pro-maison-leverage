...
const DailyBreakdownTable = ({ 
  ...
  return (
    <Card className="p-6 border-border/50 bg-card/50">
      ...
      {/* Summary */}
      <div className="mb-4 p-4 rounded-lg bg-gradient-to-r from-primary/10 to-secondary/10 border border-border/30">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
          <div>
            <div className="text-2xl font-bold text-blue-400">{totals.firstDMs}</div>
          ...
          <div>
            <div className={`text-2xl font-bold ${difference >= 0 ? 'text-green-400' : 'text-destructive'}`}>
              {difference >= 0 ? '+' : ''}{difference}
            </div>
            <div className="text-xs text-muted-foreground">Différence</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-primary">{totals.quotasMet}/{totals.workDays}</div>
            ...
            {dailyStats.map((day) => (
              ...
              >
                ...
                <TableCell className="text-center">
                  <span className={`font-bold ${day.firstDMs >= dailyTarget ? 'text-green-400' : 'text-blue-400'}`}>
                    {day.firstDMs}
                  </span>
                </TableCell>
                <TableCell className="text-center text-cyan-400">{day.followUps}</TableCell>
                <TableCell className="text-center text-yellow-400">{day.replies}</TableCell>
                <TableCell className="text-center text-green-400">{day.calls}</TableCell>
                <TableCell className="text-center">
                  {day.isFuture ? (
      ...
      {/* Funnel summary */}
      <div className="mt-4 p-4 rounded-lg bg-card border border-border/30">
        <h3 className="text-sm font-medium text-muted-foreground mb-2">Funnel de la période</h3>
        <div className="flex items-center gap-2 text-sm flex-wrap">
          <span className="font-bold text-blue-400">{totals.firstDMs} DM</span>
          <span className="text-muted-foreground">→</span>
          <span className="font-bold text-yellow-400">{totals.replies} réponses</span>
          <span className="text-xs text-muted-foreground">
            ({totals.firstDMs > 0 ? ((totals.replies / totals.firstDMs) * 100).toFixed(1) : 0}%)
          </span>
          <span className="text-muted-foreground">→</span>
          <span className="font-bold text-green-400">{totals.calls} R1</span>
          <span className="text-xs text-muted-foreground">
            ({totals.replies > 0 ? ((totals.calls / totals.replies) * 100).toFixed(1) : 0}%)
          </span>
          <span className="text-muted-foreground">→</span>
          <span className="font-bold text-purple-400">{totals.deals} deals</span>
        </div>
      </div>
    </Card>
  );
